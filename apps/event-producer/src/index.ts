...
const batchEndpoint = `${collectorUrl}/api/v1/collector/events/batch`;
...
function generateEvent(tenantId: string) {
  ...
  return {
    ...
    metadata: {
      ...
      logSource: `source-${rand(1, 10)}`,
    },
  };
}

function generateBatch(tenantId: string, size: number) {
  const events = [];
  for (let i = 0; i < size; i++) {
    events.push(generateEvent(tenantId));
  }
  return {
    tenantId: tenantId,
    events: events,
...
async function sendBatch(tenantId: string): Promise<void> {
  const batch = generateBatch(tenantId, batchSize);
  ...
  try {
    ...
    if (data) {
      tenantStat.totalSent += data.received || 0;
      tenantStat.totalAccepted += data.accepted || 0;
      tenantStat.totalDropped += data.dropped || 0;
      tenantStat.lastDecision = data.decision || "UNKNOWN";
    }
  } catch (error: any) {
    tenantStat.errors++;
...
function reportStats(): void {
  ...
  for (const tid of tenantIds) {
    const s = stats[tid];
    console.log(
      `  [${tid}] sent=${s.totalSent} accepted=${s.totalAccepted} dropped=${s.totalDropped} errors=${s.errors} lastDecision=${s.lastDecision}`
    );
  }
  console.log("════════════════════════════\n");
}
...
console.log("╔══════════════════════════════════════════╗");
console.log("║        SOC Event Producer Started        ║");
console.log("╚══════════════════════════════════════════╝");
console.log("Config:", {
  tenantIds,
  eps,
