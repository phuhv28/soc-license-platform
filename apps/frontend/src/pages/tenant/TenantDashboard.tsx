...
export default function TenantDashboard() {
  ...
  const fetchData = async () => {
    ...
    try {
      const [usageData, historyData, licensesData, alertsData] = await Promise.all([
        ...
        usageApi.getHistory(tenantId, 24),
  ...
  // Downsample history for chart (show every 5th point to keep chart readable)
  const chartData = history?.dataPoints
    ? history.dataPoints.filter((_, i) => i % 5 === 0).map(dp => ({
              ...
              {chartData.length > 0 ? (
                <ResponsiveContainer width="100%" height={300}>
                  <LineChart data={chartData}>
                    ...
                    <XAxis dataKey="time" stroke="var(--color-text-muted)" fontSize={11} interval="preserveStartEnd" />
                    ...
                    />
                    <Legend />
                    <Line type="monotone" dataKey="received" stroke="var(--color-chart-received)" strokeWidth={2} dot={false} />
                    <Line type="monotone" dataKey="accepted" stroke="var(--color-chart-accepted)" strokeWidth={2} dot={false} />
                    <Line type="monotone" dataKey="dropped" stroke="var(--color-chart-dropped)" strokeWidth={2} dot={false} />
                  </LineChart>
                </ResponsiveContainer>
              ) : (
              ...
              {activeLicense ? (
                <div className="data-table-wrapper">
                  <table className="data-table">
                    <tbody>
                      <tr><td style={{ color: 'var(--color-text-muted)' }}>EPS Quota</td><td><strong>{activeLicense.epsQuota}</strong></td></tr>
